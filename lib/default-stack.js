  ...
  optionDefinitions () {
    return [
      ...
      },
      {
        name: 'fail',
        type: String,
        description: 'fails with status code specified this way: `<Status code>:<folder1>[,<folder2>]*`'
      }
    ]
  }
      ...
      if (fs.existsSync(filepath)) {
        ...
        if (config.fail) {
          ...
          for (const split of splitted) {
            if (filepath.startsWith(config.directory ? path.join(config.directory, split): split)) {
